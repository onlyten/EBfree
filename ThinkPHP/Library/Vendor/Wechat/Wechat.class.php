...
class Wechat {
	...
	/* 上传卡券logo地址 */
	const LOGO_UPLOAD_URL         = 'https://api.weixin.qq.com/cgi-bin/media/uploadimg';
	/* 创建卡券地址 */
	const CARD_CREATE_URL         = 'https://api.weixin.qq.com/card/create';
	/* 核销卡券地址 */
	const CARD_CONSUME_URL		  = 'https://api.weixin.qq.com/card/code/consume';
	 ...
	 */
	public function orderSHA1($params) {
		// 是字符串排序，不是键名排序
	...
	public function getJsapiTicket() {
		...
		if(!$jsapiticket){
			...
			$jsonArr = $this->parseJson($jsonStr);
			if ($jsonArr) {
				$jsapiticket=$this->result['ticket'];
				S('JS_API_TICKET',$jsapiticket,7000);
				return $jsapiticket;
			}else {
				return false;
			}
	 ...
	 * 返回随机填充的字符串
	 */
	protected function _getRandomStr($lenght = 16)	{
	...
	/**
	 * 调用上传LOGO接口将商户图标上传微信服务器并获取logo_url，用于创建卡券
	 * @authors nkuwilson (nkuwilson@qq.com)
	 * @date   2016-03-26
	 * @param  [type]     $file [$file要求是绝对路径]
	 * @return [type]     [description]
	 */
	public function logo_upload($file)
	{
		$url    = self::LOGO_UPLOAD_URL . '?access_token=' . $this->access_token;
		$params = array(
			'media' => '@' . $file
		);
		$jsonStr = $this->http($url, $params, 'POST');
	 ...
	 */
	public function coupon_color_code($color)
	{
		switch ($color) {
			case '#63b359':
				$color_code = "Color010";
	 ...
	 * @authors nkuwilson (nkuwilson@qq.com)
	 * @date   2016-04-04
	 * @param  [type]     $data      [用户动态配置的数据]
	 * @param  [type]     $card_type [卡券类型，这里只添加了现金]
	 * @return [type]                [创建是否成功，bool]
	 */
	public function coupon_add($data,$card_type)
	{
		$url    = self::CARD_CREATE_URL . '?access_token=' . $this->access_token;
		/* 根据卡券类型，设定不同的卡券配置参数 */
		switch($card_type){
			case 'cash':
				$params = array("card" => array(
		...
		$str = $this->json_encode($params);
		$jsonStr = $this->http($url, $str, 'POST');
		$jsonArr = $this->parseJson($jsonStr);
		if ($jsonArr) {
			return $jsonStr;
		}else {
	...
	public function coupon_consume($code = array())
	{
		$url    = self::CARD_CONSUME_URL . '?access_token=' . $this->access_token;
		foreach ($code as $key => $value) {
			$params = array("code" => $value);
			$str = $this->json_encode($params);
			$jsonStr = $this->http($url, $str, 'POST');
			dump($jsonStr);
			$jsonArr = $this->parseJson($jsonStr);
